...
module.exports = (app, db) => {
    ...
    app.post('/api/login', async (req, res) => {
        ...
        try {
            ...
            const getCode = await db.oneOrNone('select code from user_detail where username = $1', [username]);
    ...
    app.post('/api/signUp', async (req, res) => {
        ...
        try {
            ...
            if (!oldUser) {
                ...
                var newCode = code.toString().substring(1,code.toString().length - 1);
                console.log('poooooo' + newCode)

                const user = await db.any('INSERT INTO user_detail (first_name, lastname, username, password, role, code) VALUES ($1, $2, $3, $4, $5, $6) returning *', [firstname, lastname, username, cryptedPassword, role, newCode]);
                console.log(user);
                const token = await jwt.sign({ user }, `secretKey`, { expiresIn: `24h` });
    ...
    app.post('/api/addQuestions', async function (req, res) {
        try {
            ...
            console.log('fffffff' + JSON.stringify(getTeacher.username));

            const checkQuestion = await db.oneOrNone('select questions from questions_table where questions = $1', [question])
            if (checkQuestion == null) {
                await db.any('insert into questions_table(questions,topic_id,teacher) values ($1,$2,$3)', [question, getTopicId.id, getTeacher.username])
